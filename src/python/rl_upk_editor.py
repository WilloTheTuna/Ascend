...
def build_reencrypted_package(original_encrypted_path: Path, modified_decrypted_bytes: bytes, provider: DecryptionProvider, output_path: Path, *, override_key: Optional[bytes] = None) -> Path:
    summary, meta, original_encrypted_data, decrypt_key = find_valid_key(original_encrypted_path, provider)
    # decrypt_key: used to READ the original structure (chunks, header).
    # encrypt_key: used to WRITE the output. If override_key is set, output uses a different key.
    encrypt_key = override_key if override_key is not None else decrypt_key
    modified_summary = parse_file_summary(io.BytesIO(modified_decrypted_bytes))
    # Always decrypt with the original file's key to correctly read the chunk structure.
    original_plain = bytearray(DecryptionProvider.decrypt_ecb(decrypt_key, original_encrypted_data))
    original_plain_full = bytearray(summary.name_offset) + original_plain
    # NOTE: meta.compressed_chunks_offset is relative to the start of the decrypted plain
    # (NOT the full file), matching how unpack_package calls parse_rl_compressed_chunks.
    ...
    encrypted_header = DecryptionProvider.encrypt_ecb(encrypt_key, bytes(header_plain))
...
def rename_name_entry(package: ParsedPackage, name_index: int, new_text: str) -> ParsedPackage:
    ...
    # (Bypassed for automated package swapping where duplicate strings are valid)
    pass
    # for entry in package.names:
